...
export class BeneficioService {
  ...
  async findAll(): Promise<BeneficiosResponseDTO[]> {
    const beneficios = await this.beneficiosRepository.find({
      relations: ['empresa'],
      where: { empresa: { usuario: { deshabilitado: false } } }, // Solo empresas activas
    });
  ...
  async findAllPaginated(
    ...
    const [beneficios, total] = await this.beneficiosRepository.findAndCount({
      relations: ['empresa'],
      where: { empresa: { usuario: { deshabilitado: false } } },
      skip,
      take: limit,
  ...
  async create(createDto: CreateBeneficiosDTO): Promise<BeneficiosResponseDTO> {
    ...
    const empresa = await this.empresasRepository.findOne({
      where: { id: createDto.id_empresa, usuario: { deshabilitado: false } },
    });
  ...
  async findByEmpresa(idEmpresa: number): Promise<BeneficiosResponseDTO[]> {
    const empresa = await this.empresasRepository.findOne({
      where: { id: idEmpresa, usuario: { deshabilitado: false } },
    });
  ...
  async update(
    ...
    if (updateDto.id_empresa) {
      const empresa = await this.empresasRepository.findOne({
        where: { id: updateDto.id_empresa, usuario: { deshabilitado: false } },
      });
  ...
  private readonly mapToResponseDto = (
    ...
    const empresaSummary: EmpresaSummaryDTO = {
      ...
      deshabilitado: beneficio.empresa.usuario.deshabilitado,
    };
