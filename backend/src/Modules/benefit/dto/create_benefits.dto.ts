...
/**
 * Data Transfer Object (DTO) especifico de Create para Benefits.
 */
export class CreateBenefitsDTO {
    @ApiProperty({ example: 'Descuento del 15%', description: 'Título del Beneficio' })
    @IsString()
    @IsNotEmpty()
    @MaxLength(255)
    titulo: string;
    ...
    @ApiProperty({ example: 'Descuento en artículos de Supermercado', description: 'Detalle del beneficio' })
    @IsString()
    @IsNotEmpty()
    @MaxLength(255)
    detalle: string;
