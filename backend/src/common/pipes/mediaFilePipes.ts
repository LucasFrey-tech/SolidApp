    ...
    transform(value: any, metadata: ArgumentMetadata) {
        ...
        for (const file of value) {
            ...
            if (file.size > this.maxSize) {
                throw new BadRequestException(`File size exceeds maximum of ${this.maxSize} bytes`);
            }

            if (!this.allowedTypes.test(file.originalname)) {
                throw new BadRequestException('Only jpg, jpeg, svg and png files are allowed');
            }
        }
